...
/// Whether a path may be written without risk of clobbering something else.
///
/// A symlink is never safe: `fs::write` follows it and truncates whatever sits
...
pub fn resolve_target(dir: &Path, date: &str) -> Result<(PathBuf, Option<String>), String> {
    let intended = dir.join(format!("{date}.md"));
    if is_writable_target(&intended) {
        return Ok((intended, None));
    }

    // The intended file is not ours. Every fallback needs the *same* check --
    // the user may well have hand-written `<date>.workstreams.md` too, and
    // writing it blind would destroy exactly what stepping aside was meant to
    // protect.
    for suffix in 0..100 {
        let candidate = if suffix == 0 {
            dir.join(format!("{date}.workstreams.md"))
        } else {
            dir.join(format!("{date}.workstreams.{suffix}.md"))
        };
        if is_writable_target(&candidate) {
            return Ok((
                candidate,
                Some(format!(
                    "{date}.md was not generated by Workstreams, so it was left \
                     untouched and the export was written alongside it."
                )),
            ));
        }
    }
    ...
    Err(format!(
        "refusing to write: {date}.md and every alongside name are files \
         Workstreams did not generate"
    ))
}
...
pub fn write_and_commit(
    ...
    // Re-verify immediately before writing. This does not close the race
    // entirely -- only an atomic no-follow open would -- but export is a
    // manual, single-user action, so the check shrinks the window to nothing
    // meaningful at no cost.
    if !is_writable_target(&path) {
        return Err(format!(
            "refusing to write {}: it changed underneath us and is no longer a file Workstreams generated",
            path.display()
        ));
    }
