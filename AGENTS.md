...
2. **Git hooks** (versioned in `.githooks/`, wired via `git config core.hooksPath` from `scripts/install-hooks.mjs`, run automatically by `npm` postinstall):
   ...
   - **Pre-push (authoritative, mirrors CI)**: `tsc --noEmit --incremental`, `vitest run --coverage` (90% threshold), `cargo clippy -D warnings`, **conditional full Playwright E2E** (`--workers=2 --retries=1`, ~90s cold; triggered when the push touches `e2e/`, `src/tiles/`, `src/files/`, `src/harness/`, `src/App.tsx`, `src/backend/`, or `playwright.config.ts` — unrelated pushes skip it), smart doc gate (>200 lines source → require doc touch or `[no-docs: reason]` bypass)
