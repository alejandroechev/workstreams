...
2. **Git hooks** (versioned in `.githooks/`, wired via `git config core.hooksPath` from `scripts/install-hooks.mjs`, run automatically by `npm` postinstall):
   ...
   - **Pre-push (authoritative, mirrors CI)**: `tsc --noEmit --incremental`, `vitest run --coverage` (90% threshold), `cargo clippy -D warnings`, **conditional Playwright E2E** (`--only-changed` vs upstream, triggered when the push touches `e2e/`, `src/tiles/`, `src/files/`, `src/harness/`, `src/App.tsx`, `src/backend/`, or `playwright.config.ts`), smart doc gate (>200 lines source → require doc touch or `[no-docs: reason]` bypass)
...
4. **Post-push CI watch (mandatory)** — the pre-push hook only runs the *affected* E2E specs (`--only-changed`); a source change can still break a spec it doesn't import (specs drive the app by URL, not by import). So **after every push, watch CI to green before treating the work as done**: `gh run watch $(gh run list --branch <branch> --limit 1 --json databaseId -q '.[0].databaseId') --exit-status`. If CI fails, fix and re-push — do not declare the task complete on a red run.
