...
2. **Git hooks** (versioned in `.githooks/`, wired via `git config core.hooksPath` from `scripts/install-hooks.mjs`, run automatically by `npm` postinstall):
   ...
   - **Pre-push (authoritative, mirrors CI)**: `tsc --noEmit --incremental`, `vitest run --coverage` (90% threshold), `cargo clippy -D warnings`, **conditional full Playwright E2E** (`--workers=1 --retries=2`, ~80-135s; triggered when the push touches `e2e/`, `src/tiles/`, `src/files/`, `src/harness/`, `src/App.tsx`, `src/backend/`, or `playwright.config.ts` — unrelated pushes skip it; runs serially because the Monaco view-zone specs flake with parallel workers), smart doc gate (>200 lines source → require doc touch or `[no-docs: reason]` bypass)
...
3. **CI mirrors the hooks** (`.github/workflows/ci.yml`): every check the pre-push hook runs also runs in CI, plus the **full** Playwright E2E suite and Rust unit tests. CI is the authoritative gate; bypassing local hooks does not bypass CI. CI never builds the release installer or creates tags — releases are manual via `.github/workflows/release.yml` ("Run workflow" button on GitHub Actions).

4. **Post-push CI watch (recommended backstop)** — the pre-push runs the full E2E suite only when UI/e2e files change; for pushes that skip it (or to catch environment-specific CI differences), **watch CI to green before treating the work as done**: `gh run watch $(gh run list --branch <branch> --limit 1 --json databaseId -q '.[0].databaseId') --exit-status`. If CI fails, fix and re-push — do not declare the task complete on a red run.
